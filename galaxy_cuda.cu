...
//    srun -p gpu -c 1 --mem=10G ./galaxy_cuda RealGalaxies_100k_arcmin.txt SyntheticGalaxies_100k_arcmin.txt omega.out
...
__global__ void  fillHistogram(float * real_rasc, float * real_decl, float * rand_rasc, float * rand_decl, unsigned long long int * histogramDR, unsigned long long int * histogramDD, unsigned long long int * histogramRR) {
	...
	if(i < 100000){
		...
		float tempDR,tempDD,tempRR = 0.0f;
	    	...
	    	for(j = i+1;j < 100000;j++){// we are doing exactly the same technique as exercise 2
			...
			atomicAdd(&histogramDR[(int)(angleDR*4.0f)],2L);//allow for 1 thread to write at a time
			atomicAdd(&histogramDD[(int)(angleDD*4.0f)],2L);
			atomicAdd(&histogramRR[(int)(angleRR*4.0f)],2L);
...
// input data is available in the arrays float real_rasc[], real_decl[], rand_rasc[], rand_decl[];
   ...
   if(cudaDeviceSynchronize() != cudaSuccess) return(-1);
    ...
    histogram_DD[0] += 100000;//we add 100000 like our old codes
    histogram_RR[0] += 100000;
	histogram_DR[0] += 100000;
