...
//  and so on until     89.75 <=  angle  <= 90.0
    ...
    #pragma omp parallel for private(j,i,angleDD,angleRR)
    for (i=0;i<100000;++i){
            for(j=i+1;j<100000;++j){
                angleDD= acosf(sinf(real_decl[i])*sinf(real_decl[j])+cosf(real_decl[i])*cosf(real_decl[j])*cosf(real_rasc[i]-real_rasc[j]))*180/pif;
                
                angleRR= acosf(sinf(rand_decl[i])*sinf(rand_decl[j])+cosf(rand_decl[i])*cosf(rand_decl[j])*cosf(rand_rasc[i]-rand_rasc[j]))*180/pif;
	...
	#pragma omp parallel for private(j,i,angleDR)
	for (i=0;i<100000;++i){
            for(j=0;j<100000;++j){
				angleDR= acosf(sinf(real_decl[i])*sinf(rand_decl[j])+cosf(real_decl[i])*cosf(rand_decl[j])*cosf(real_rasc[i]-rand_rasc[j]))*180/pif;
